...
options! {
    ...
    /// Output path for a symbol list file (one symbol per line).
    emit_symbol_list: Option<PathBuf> {
        methods: {
            /// Set the path for a file where `bindgen` will emit a list of all generated symbols.
            ///
            /// This is useful for downstream crates that depend on bindings generated by this
            /// crate: they can read this file and use it to blocklist symbols that have already
            /// been bound.
            ///
            /// The output format is one symbol per line. Each line has the format:
            /// `KIND NAME` where KIND is one of `type`, `function`, or `var`.
            ///
            /// This list is not emitted by default.
            pub fn emit_symbol_list<P: Into<PathBuf>>(mut self, path: P) -> Builder {
                self.options.emit_symbol_list = Some(path.into());
                self
            }
        },
        as_args: "--emit-symbol-list",
    },
