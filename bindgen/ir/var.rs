...
/// Possible integer literal suffices, all cases (U|u)?(L|l){0,2}, from longest to
/// shortest in number of characters.
#[rustfmt::skip]
const INTEGER_SUFFICES: &[&[u8]] = &[
    b"ULL", b"ULl", b"UlL", b"Ull", b"uLL", b"uLl", b"ulL", b"ull",
    b"UL", b"Ul", b"uL", b"ul",
    b"LL", b"Ll", b"lL", b"ll",
    b"U", b"u",
    b"L", b"l",
...
impl LiteralRadix {
    /// Obtain the number base of a bytestring corresponding to an existing integer
    /// literal definition.
    ///
    ...
    pub(crate) fn from_integer_literal_token(
        ...
        let tok = INTEGER_SUFFICES
            .iter()
            .find_map(|suffix| tok.strip_suffix(*suffix))
            .expect("last suffix is empty bytestring");

        if tok.is_empty() {
            return None;
        } else if tok.len() == 1 {
            // single digit numbers from 0 to 9 are always decimal
...
mod test {
    ...
    #[test]
    fn parses_none_radix_from_lone_integer_suffices() {
        for suffix in INTEGER_SUFFICES {
            assert_eq!(
