...
impl Cursor {
    ...
    /// Returns `true` if `other`'s source range lies fully within
    /// `self`'s source range.
    ///
    ...
    pub(crate) fn extent_contains(&self, other: &Cursor) -> bool {
        unsafe {
            ...
            let child_range = clang_getCursorExtent(other.x);
            ...
            let mut child_end_off: c_uint = 0;
