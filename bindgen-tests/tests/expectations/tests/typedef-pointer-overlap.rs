#![allow(dead_code, non_snake_case, non_camel_case_types, non_upper_case_globals)]
#[repr(C)]
#[derive(Debug, Default, Copy, Clone)]
pub struct foo {
    pub inner: ::std::os::raw::c_char,
}
const _: () = {
    assert!(::std::mem::size_of::<foo>() == 1usize, "Size of foo");
    assert!(::std::mem::align_of::<foo>() == 1usize, "Alignment of foo");
    assert!(::std::mem::offset_of!(foo, inner) == 0usize, "Offset of field: foo::inner");
};
pub type foo_ptr = *const foo;
#[repr(C)]
#[derive(Debug, Default, Copy, Clone)]
pub struct bar {
    pub inner: ::std::os::raw::c_char,
}
const _: () = {
    assert!(::std::mem::size_of::<bar>() == 1usize, "Size of bar");
    assert!(::std::mem::align_of::<bar>() == 1usize, "Alignment of bar");
    assert!(::std::mem::offset_of!(bar, inner) == 0usize, "Offset of field: bar::inner");
};
pub type bar_ptr = *mut bar;
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct baz {
    _unused: [u8; 0],
}
pub type baz_ptr = *mut baz;
#[repr(C)]
#[derive(Copy, Clone)]
pub union cat {
    pub standard_issue: ::std::os::raw::c_int,
}
const _: () = {
    assert!(::std::mem::size_of::<cat>() == 4usize, "Size of cat");
    assert!(::std::mem::align_of::<cat>() == 4usize, "Alignment of cat");
    assert!(
        ::std::mem::offset_of!(cat, standard_issue) == 0usize,
        "Offset of field: cat::standard_issue",
    );
};
impl Default for cat {
    fn default() -> Self {
        let mut s = ::std::mem::MaybeUninit::<Self>::uninit();
        unsafe {
            ::std::ptr::write_bytes(s.as_mut_ptr(), 0, 1);
            s.assume_init()
        }
    }
}
pub type cat_ptr = *mut cat;
pub const mad_scientist: mad = 0;
pub type mad = ::std::os::raw::c_uint;
pub type mad_ptr = *mut mad;
extern "C" {
    pub fn takes_foo_ptr(arg1: foo_ptr);
}
extern "C" {
    pub fn takes_foo_struct(arg1: foo);
}
extern "C" {
    pub fn takes_bar_ptr(arg1: bar_ptr);
}
extern "C" {
    pub fn takes_bar_struct(arg1: bar);
}
extern "C" {
    pub fn takes_baz_ptr(arg1: baz_ptr);
}
extern "C" {
    pub fn takes_baz_struct(arg1: baz);
}
extern "C" {
    pub fn takes_cat_ptr(arg1: cat_ptr);
}
extern "C" {
    pub fn takes_cat_union(arg1: cat);
}
extern "C" {
    pub fn takes_mad_ptr(arg1: mad_ptr);
}
extern "C" {
    pub fn takes_mad_enum(arg1: mad);
}
