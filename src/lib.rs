...
impl Builder {
    ...
    pub fn generate(mut self) -> Result<Bindings, ()> {
        if let Some(bin) = env::var_os("BINDGEN_USE_BINARY") {
            let output = Command::new(bin)
                .args(self.command_line_flags())
                .env_remove("BINDGEN_USE_BINARY")
                .output()
                .expect("Failed to run BINDGEN_USE_BINARY");
            return Ok(Bindings(BindingsInner::Eager(String::from_utf8(output.stdout).expect("Bindgen should only generate valid utf-8"))));
        }
...
#[derive(Debug)]
pub struct Bindings(BindingsInner);
...
impl Bindings {
    ...
    pub(crate) fn generate(
        ...
        Ok(Bindings(BindingsInner::Lazy {
            options,
            module: quote! {
                #( #items )*
            },
    ...
    pub fn to_string(&self) -> String {
        if let BindingsInner::Eager(ref s) = self.0 {
            return s.clone()
        }
    ...
    pub fn write<'a>(&self, mut writer: Box<dyn Write + 'a>) -> io::Result<()> {
        let (options, module) = match self.0 {
            BindingsInner::Eager(ref s) => return writer.write_all(s.as_bytes()),
            BindingsInner::Lazy { ref options, ref module } => (options, module),
        };

        if !options.disable_header_comment {
        ...
        for line in options.raw_lines.iter() {
            writer.write_all(line.as_bytes())?;
            writer.write_all("\n".as_bytes())?;
        }

        if !options.raw_lines.is_empty() {
            writer.write_all("\n".as_bytes())?;
        }

        let bindings = module.to_string();

        match self.rustfmt_generated_string(&bindings, options) {
            Ok(rustfmt_bindings) => {
    ...
    fn rustfmt_path<'a>(options: &'a BindgenOptions) -> io::Result<Cow<'a, PathBuf>> {
        debug_assert!(options.rustfmt_bindings);
