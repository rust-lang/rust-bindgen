...
/// Enum for how non-Rust unions should be translated.
#[derive(Copy, Clone, PartialEq, Debug)]
pub enum NonRustUnionStyle {
...
impl NonRustUnionStyle {
    /// Convert an `NonRustUnionStyle` to its str representation.
...
impl Default for NonRustUnionStyle {
    fn default() -> Self {
        Self::BindgenWrapper
    }
}

impl std::str::FromStr for NonRustUnionStyle {
    type Err = std::io::Error;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            _ => Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                concat!(
                    "Got an invalid NonRustUnionStyle. Accepted values ",
